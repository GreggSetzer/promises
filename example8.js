// Example 8: Running multiple promises at once for better performance.
...
function getShippingDetails(customer) {
   return new Promise(resolve => {
      ...
      const customerShippingData = shippingDetails.filter(shippingData => {
         return shippingData.customerId === customer.customerId;
      });

      resolve(customerShippingData);
   });
}
...
getCustomer().then((customer) => {
   const promises = [
      getOrders(customer), // promise #0
      getShippingDetails(customer) // promise #2
   ];

   Promise.all(promises)
      .then((myArr) => {
         customer.orders = myArr[0];
         customer.shippingData = myArr[1];
