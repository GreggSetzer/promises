...
function getShippingDetails(customer) {
   return new Promise(resolve => {
      ...
      const details = shippingDetails.filter(shippingData => {
...
async function buildCustomerOrder() {
   try {
      const customer = await getCustomer();
      customer.orders = await getOrders(customer);
      customer.shippingData = await getShippingDetails(customer);
